...
const string fileName = "Testing.csv";						//This is where you put the name of the .csv file you want read.
...
int main()													//instructions for how the file must be formatted are in parse.h and parse.cpp
{
	Division europe;
	Division NA;
	int count = 18;											//the exact number or players in the .csv file.
	parse(europe, NA, count, fileName);
	NA.tallyRoles();
	cout << (NA.search(2100, 2200, ROLE_ANY));
    return 0;
}
